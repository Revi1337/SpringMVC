...
public class SessionInfoController {
    ...
    public String sessionInfo(HttpServletRequest httpServletRequest) {
        ...
        log.info("세션이 생성된 시간 = {}", new Date(session.getCreationTime()));
        log.info("세션의 유효시간 (중요) = {}", session.getMaxInactiveInterval()); // session.getLastAccessedTime() 와 관련있음.
        log.info("세션에 마지막으로 접근한 시간 (중요) = {}", new Date(session.getLastAccessedTime()));
        log.info("새로 생성된 세션이냐 = {}", session.isNew());
